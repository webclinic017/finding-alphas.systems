...
def app():
    ...
    if (st.button('Execute backtest')):
        ...
        with st.spinner("Plotting backtest statistics ..."):
            ...
            # cs_risk_plot = momentum.cs_risk_weight.plot.area(figsize=(17, 7), title=('Cross-Sectional Weights'))
            st.area_chart(momentum.cs_risk_weight)
            # st.pyplot(cs_risk_plot)

            # Time-Series Weights
            # replace weird numbers (x > 1) to 1
            ts_risk_plot = momentum.ts_risk_weight.map(lambda x: 1 if x > 1 else x) \
                .plot.area(figsize=(17, 7), title='Vol-Targeting Weight')
            # st.pyplot(ts_risk_plot)
            st.area_chart(momentum.ts_risk_weight.map(lambda x: 1 if x > 1 else x))
